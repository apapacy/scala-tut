...
object OrientDbSample extends App {
  ...
  val factory: OrientGraphFactory = new OrientGraphFactory(uri, "root", "password")
  ...
  try {
    ...
    if (graph.getVertexType("Person") == null) {
      ...
      work.createProperty("projects", OType.LINKSET, project)
    }
    else {
    ...
    var x = 0
    var fut = mutable.MutableList();
      for (x <- 1 to 100000) {
        fut +=  (Future {
          graph.addVertex("class:Person", "firstName", "Jane" + x, "lastName", "Doe" + x)
        })
      }

    Await.result(fut, 10000.seconds)
    // creates a Company
    ...
    johnDoeAcme.setProperty("projects", Array(acmeGlue, acmeGlue, acmeRocket))
